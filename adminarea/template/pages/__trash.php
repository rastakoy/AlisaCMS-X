...
?>
<div class="ui-state-default-2 connectedSortable" id="prm_?action=lookItem,trashOption=<?=$option['link']?>,option=<?=$params['option']?>,parents=<?=$params['parents']?>,itemId=<?=$item['id']?>">
	...
	?>background-color:#CCCCCC;<? } ?>">
		<table cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
			...
			<td height="34" width=""
			style="font-weight:bold;font-size:11px;"><span id="sn_<?=$option['link']?>_<?=$item['id']?>"><?=$item['name']?></span></td>
			<td height="34" width="20">&nbsp;</td>
			<td height="34" width="20">&nbsp;</td>
			<td height="34" width="20"><a href="javascript:" title="Внимание: незаполненные поля">&nbsp;</a></td>
			...
			<td height="34" width="20"><a href="javascript:" title="Удалить запись"><img src="/adminarea/template/images/green/myitemname_popup/delete_item.gif" id="imgoptions_105" width="16" height="16" border="0" align="right" style="margin-right:5px;cursor:pointer;margin-top:5px;" onclick="completelyRemove('<?=$option['link']?>', '<?=$item['id']?>')"></a></td>
		</tr></table>
	</div>
</div>
<? }}} ?>
...
<script>
//**********************************
function completelyRemove(option, itemId){
	var name = document.getElementById("sn_"+option+"_"+itemId).innerHTML;
	if(confirm("Вы желаете безвозвратно удалить запись\n«"+name+"»")){
		var paction =  "ajax=completelyRemove";
		...
		$.ajax({
			...
			success: function(html) {
				console.log(html);
				getData(__GLOBALS.adminBase+"/?option=trash");
			}
		});
	}
}
