...
class Core extends DatabaseInterface{
	...
	*/
	function makePost($array){
		...
		$login = $array['adlogin'];
		if(!$login){
			$login = $_SESSION['adlogin'];
		}
		$pass = md5($array['adpassword']);
		if(!$pass){
			$pass = $_SESSION['adpassword'];
		}
	...
	*/
	function constructSiteMenus(){
		$query = $this->query("SELECT * FROM `menus` WHERE `folder`='1' ORDER BY `id` ASC ");
	...
	*/
	function constructSiteBlocks(){
		$query = $this->query("SELECT * FROM `blocks` ORDER BY `id` ASC ");
		while($block=$query->fetch_assoc()){
			$blocks[] = $block;
		}
		return $blocks;
	}
