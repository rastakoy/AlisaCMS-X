...
class Data extends DatabaseInterface{
	...
	*/
	function completelyRemove($array){
		$option = $array['option'];
		$itemId = $array['itemId'];
		$query = $this->query("SELECT * FROM `$array[option]` WHERE `id`='$array[itemId]' AND `trash`='1' ");
		if($query->num_rows > 0){
			$item = $query->fetch_assoc();
			if($item['folder']=='1'){
				$type = "folder";
		...
		if($type=='item'){
			$classImages->removeImage($option, $itemId);
			$query = $this->query("DELETE FROM `$option` WHERE `id`='$itemId' ");
		}
	}
	...
	*/
	function getElementBySiteURL($array){
		$level = '1';
		$parent = '0';
