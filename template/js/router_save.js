...
$(document).ready(function(){
	...
	for(var j=0; j<objs.length; j++){
		...
		obj.onclick = function(){
			...
			id = id.replace(/\/$/gi, '');
			if(document.getElementById(id+"_left_menu")){
				if(document.getElementById(id+"_left_menu").style.display=="none"){
					getPage(this.getAttribute("href"));
					addLeftBranchRed();
					document.getElementById(id+"_left_menu").style.display="";
				}else{
					document.getElementById(id+"_left_menu").style.display="none";
				}
			}else{
				getPage(this.getAttribute("href"));
				addLeftBranchRed();
...
if ( window.addEventListener ) {
	window.addEventListener('popstate', function( e ) {
		...
		if(gurl){
			getPage(window.history.state.url);
		}
	}, false);
}
//************************************************
function constructParams(str, newParam, newValue){
	...
	for(var j=0; j<mass.length; j++){
		var foo = mass[j].split('=');
		ret[foo[0]] = foo[1];
	}
	if(newParam){
		ret[newParam] = newValue;
	}
...
function getPage(url, newParam, newValue){
	//alert(url);
	if(globalEdit){
	...
	if(params['action']=='editItem' && !params['itemId']){
		var paction = "ajax=addLeftMenuNewItem&parent="+params['parent']+"&option="+url.replace(/\/.*$/gi, '');
		...
		$.ajax({
			...
			success: function(html) {
				...
				getPage(window.location.pathname+"?action=editItem,parent="+data.parent+",itemId="+data.itemId);
			}
		});
		return false;
	}
...
function initItems(){
	...
	$( ".div_myitemname" ).dblclick(function () {
		...
		//alert(gurl+this.parentNode.id.replace(/prm_/, ""));
		if(this.parentNode.id.match(/\?/gi)){
			getPage(gurl+this.parentNode.id.replace(/prm_/, ""));
		}else{
			getPage(gurl+this.parentNode.id.replace(/prm_/, "")+"/");
		}
...
function savePriors(url){
	...
	for(var j=0; j<priors.length;j++){
		if(priors[j].match(/\?/gi)){
			...
			myMatch = foo.match(/itemId=[0-9]{1,11}/);
			priors[j] = "prm_"+myMatch[0].replace(/itemId=/gi, '');
		}
	}
	//alert(priors);
	$.ajax({
		type: "POST",
		url: __ajax_url,
		data: "ajax=savePriors&url="+encodeURIComponent(url)+"&ids="+priors,
		success: function(html) {
			//alert(html);
			var data = eval("("+html+")");
			if(data.parent==''){
				getLeftBranch(data.table);
			}else{
				if(document.getElementById("notItem_"+data.table+"_"+data.parent)){
					var img = document.getElementById("notItem_"+data.table+"_"+data.parent).getElementsByTagName("img")[0];
					if(img.src.match(/minus\.jpg$/)){
						img.src = img.src.replace(/minus\.jpg$/gi, 'plus.jpg');
						element = document.getElementById("notItem_"+data.table+"_"+data.parent).getElementsByTagName("ul")[0];
						element.parentNode.removeChild(element);
					}
				}
				openLeftBranch(data.table, data.parent);
			}
		}
	});
}
